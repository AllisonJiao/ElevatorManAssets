...
SCENE_ASSET_PATH = "ElevatorManAssets/assets/SceneDemo.usdc"
...
def design_scene() -> dict:
    ...
    light_cfg.func("/World/env_light", light_cfg)
...
def run_simulator(sim: sim_utils.SimulationContext, scene_entities: dict):
    ...
    camera_positions = torch.tensor(
        [
            [ 1.8, -2.2, 1.6],   # Camera 0: outside elevator, looking in (1st screenshot)
            [-2.4,  0.0, 1.6],   # Camera 1: side view on robot & panel (2nd screenshot)
        ],
        device=sim.device,
    )

    # Both cameras look at about the same point (center of button panel / robot upper body)
    camera_targets = torch.tensor(
        [
            [0.0, 0.0, 1.2],     # target for cam 0
            [0.0, 0.0, 1.2],     # target for cam 1
        ],
        device=sim.device,
    )
